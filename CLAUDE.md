...
indicator, Drought**. It takes EPA's published per-figure CSV downloads, in
...
`narrative.qmd` is lifted into `indicators/drought.qmd` there.

**This repository is not a website and draws no figures.** All chart code lives
in the site repository, in `R/drought.R`. Nothing here should produce a plot, a
